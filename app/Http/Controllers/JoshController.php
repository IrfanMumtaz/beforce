...
//    }
    ...
    public function showHome(Request $request)
    {
    	...
    	//total Employee
    	$emp=Employee::where([['deleted_at',NULL],['EmployeeStatus','Active']])->get();
    	$emp_count=$emp->count();
        $date=date('Y-m-d');
        $attendance_result=DB::table('attendance')->whereBetween('created_at', [$date.' 00:00:00', $date.' 23:59:59'])->get();
        $emp_ids=array();
        foreach ($attendance_result as $key => $value) {
            $emp_ids[]=$value->empid;
        }
        $present_emps=count(Employee::whereIn('id',$emp_ids)->get());
